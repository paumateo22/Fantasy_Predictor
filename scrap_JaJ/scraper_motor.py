...
def scrap_datos_entrenador(url, jornada, temporada):
    ...
    try:
        ...
        marcador_local = soup.select_one('.team.local .name')
        marcador_visit = soup.select_one('.team.visitante .name')
        
        local = marcador_local.text.strip() if marcador_local else "Local"
        visitante = marcador_visit.text.strip() if marcador_visit else "Visitante"
